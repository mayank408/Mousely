...
while(cap.isOpened()):
	...
	if ret==True:
		...
		faces = cv2.CascadeClassifier('res/haarcascade_eye.xml')
		...
		for (x,y,w,h) in detected:
			...
			contours, hierarchy,_= cv2.findContours(threshold,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
			...
			if len(contours) >= 2: 
				...
				for cnt in contours:
					ctr = np.array([[[0,0]], [[10,0]], [[10,10]], [[5,4]]])
